...
def gen_config(cmakedir, cli_args):
    ...
    args += [
        '-DHAVE_{}={}'.format(ARG_TO_HAVE_MAP.get(pkg, pkg), have_value(pkg))
    ...
    # Handle BUILD_* flags
    args += [
        '-DBUILD_{}={}'.format(comp, 'ON' if comp in cli_args.pkgs else 'OFF')
...
def main():
    ...
    cmakedir = os.path.abspath(os.path.join(
